...
namespace MozaikaApp.Models
{
    ...
    public class RC6 // 32/20/16 - 128/192/256
    {
        ...
        private static readonly uint[] RoundKey = new uint[2 * R + 4];  // ключ раунда
        private const int W = 32; // длина машинного слова в битах
        private static byte[] _mainKey; // ключ
        ...
        private static void GenerateKey(int @long, byte[] keyCheck)
        {
            ...
            if (keyCheck == null)
            {
                ...
                _mainKey = aesCrypto.Key;
            }
            else _mainKey = keyCheck;
            int c = 0;
            int i, j;
            ...
            for (i = 0; i < c; i++)
            {
                L[i] = BitConverter.ToUInt32(_mainKey, i * 4); // разбиваем ключ на слова
            }
        ...
        public byte[] EncodeRc6(string plaintext)
        {
            ...
            for (i = 0; i < text.Length; i = i + 16)
            {
                ...
                uint[] tempWords = new uint[] { A, B, C, D };
